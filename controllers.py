...
def create_schedule():
    ...
    for i in matches.index:
        possible_squads =  schedule.loc[(schedule['Shift End'] > matches.loc[i, "Availability"]) & (schedule['Shift End'] - matches.loc[i, "Availability"] >= pd.to_timedelta(4, unit='h'))]
        ...
        for key, _ in shifts_group:
            ...
            prefered_squad = shift[(shift['Squad'] == matches.loc[i, 'Squad']) & (shift['Quantity'] > 0)]
            ...
            if not prefered_squad.empty:
                ...
                if prefered_squad.loc[prefered_squad_index, 'Shift End'] - matches.loc[i, "Availability"] >= pd.to_timedelta(8, unit='h'):
                    ...
                    # if prefered_squad.loc[prefered_squad_index, 'Quantity'] == 0:
                    #     break
            ...
            for index in shift.index:
                ...
                if matches.loc[i, 'Remaining Hours'] == 0:
                    ...
                    elif shift.loc[index, 'Quantity'] >= 1: # check on quantity instead of hours
                        matches.at[i, 'Remaining Hours'] = matches.loc[i, 'Remaining Hours'] - 4
                    ...
                    if index == shift.index[-1] & len(output.loc[(output['Match ID'] == matches.loc[i, 'ID'])].index) == 0:
                        output = pd.concat([output, pd.DataFrame({"Match ID": [matches.loc[i, 'ID']],
                                                        'Match Availability':[matches.loc[i, 'Availability']],
                                                        'Match Deadline':[matches.loc[i, 'Deadline']],
                                                        "Done": [False]})])
